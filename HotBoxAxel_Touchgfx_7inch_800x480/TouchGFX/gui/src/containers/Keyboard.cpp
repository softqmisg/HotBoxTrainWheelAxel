...
Keyboard::Keyboard():
Keyboard(HAL::DISPLAY_WIDTH,HAL::DISPLAY_HEIGHT)
{

}
Keyboard::Keyboard(uint16_t width=HAL::DISPLAY_WIDTH,uint16_t height=HAL::DISPLAY_HEIGHT)
:   m_keys(), m_colorScheme(),
	...
	keyboard_width(width),keyboard_height(height),
	keypressTimout(10000),
	counterTimout(0),
	startTimer(false)
{
...
{
	initialize(keyboard_width, keyboard_height);
}
void Keyboard::initialize(uint16_t width,uint16_t height)
{
	    ...
	    const uint16_t h = keyboard_height;     // Screen height.
	    const uint16_t w = keyboard_width;      // Screen width.
