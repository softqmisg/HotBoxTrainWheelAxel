...
static void W25QXXX_QSPI_WaitForWriteEnable(QSPI_HandleTypeDef *hqspi);
static void W25QXXX_QSPI_WaitForBusy(QSPI_HandleTypeDef *hqspi);
...
uint8_t W25QXXX_QSPI_Init(QSPI_HandleTypeDef *hqspi)
{
 ...
 /* QSPI memory reset */
  if (W25QXXX_QSPI_ResetMemory(hqspi) != QSPI_OK)
  {
    return QSPI_NOT_SUPPORTED;
  }
  HAL_Delay(1);

  /*if (W25QXXX_QSPI_WriteEnable(hqspi)!= QSPI_OK) {
	  return QSPI_NOT_SUPPORTED;
  }*/

	if(W25QXXX_QSPI_AutoPollingMemReady(hqspi, W25QXXX_SUBSECTOR_ERASE_MAX_TIME) != QSPI_OK){
 ...
 if (W25QXXX_QSPI_EnterFourBytesAddress(hqspi) != QSPI_OK)
  {
    return QSPI_NOT_SUPPORTED;
  }

   /*if (W25QXXX_QSPI_EnableQspi(hqspi) != QSPI_OK){
  	 return QSPI_NOT_SUPPORTED;
   }*/

  /* Configuration of the dummy cycles on QSPI memory side */
  if (W25QXXX_QSPI_DummyCyclesCfg(hqspi) != QSPI_OK)
  {
    return QSPI_NOT_SUPPORTED;
  }

 /*if (W25QXXX_QSPI_WriteDefaultStatusREGs(hqspi) != QSPI_OK){
	 return QSPI_NOT_SUPPORTED;
 }*/
...
{
  QSPI_HandleTypeDef W25QXXX_Qspi = *QSPIHandle;
  ...
  MODIFY_REG(W25QXXX_Qspi.Instance->DCR, QUADSPI_DCR_CSHT, QSPI_CS_HIGH_TIME_3_CYCLE);
  ...
  MODIFY_REG(W25QXXX_Qspi.Instance->DCR, QUADSPI_DCR_CSHT, QSPI_CS_HIGH_TIME_6_CYCLE);
...
uint8_t W25QXXX_QSPI_Write(QSPI_HandleTypeDef *QSPIHandle,uint8_t* pData, uint32_t WriteAddr, uint32_t Size)
{
  ...
  current_size = W25QXXX_PAGE_SIZE - (WriteAddr % W25QXXX_PAGE_SIZE);
...
{
  QSPI_CommandTypeDef s_command;
  EraseStartAddress = EraseStartAddress - EraseStartAddress % W25QXXX_SECTOR_SIZE;
  ...
  EraseStartAddress += W25QXXX_SECTOR_SIZE;

  /* Configure automatic polling mode to wait for end of erase */

  if (W25QXXX_QSPI_AutoPollingMemReady(QSPIHandle, W25QXXX_SUBSECTOR_ERASE_MAX_TIME) != QSPI_OK)
  {
     return QSPI_ERROR;
  }
...
{
  QSPI_CommandTypeDef s_command;
  EraseStartAddress = EraseStartAddress - EraseStartAddress % W25QXXX_BLOCK_SIZE;
  ...
  EraseStartAddress += W25QXXX_BLOCK_SIZE;
...
uint8_t W25QXXX_QSPI_Erase_Chip(QSPI_HandleTypeDef *QSPIHandle)
{
  ...
  /* Configure automatic polling mode to wait for end of erase */
  if (W25QXXX_QSPI_AutoPollingMemReady(QSPIHandle, W25QXXX_BULK_ERASE_MAX_TIME) != QSPI_OK)
  {
      return QSPI_ERROR;
  }
...
uint8_t W25QXXX_QSPI_GetStatus(QSPI_HandleTypeDef *QSPIHandle)
{
  ...
  if ((reg & (W25QXXX_FSR_PRERR | W25QXXX_FSR_VPPERR | W25QXXX_FSR_PGERR | W25QXXX_FSR_ERERR)) != 0)
  {
    return QSPI_ERROR;
  }
  else if ((reg & (W25QXXX_FSR_PGSUS | W25QXXX_FSR_ERSUS)) != 0)
  {
    return QSPI_SUSPENDED;
  }
  else if ((reg & W25QXXX_FSR_READY) != 0)
  {
    return QSPI_OK;
  }
  else
...
uint8_t W25QXXX_QSPI_GetInfo(QSPI_Info* pInfo)
{
  ...
  pInfo->EraseSectorSize    = W25QXXX_SECTOR_SIZE;
  pInfo->EraseSectorsNumber = (W25QXXX_FLASH_SIZE/W25QXXX_SECTOR_SIZE);
...
uint8_t W25QXXX_QSPI_ResetMemory(QSPI_HandleTypeDef *hqspi)
{
  ...
  if (W25QXXX_QSPI_AutoPollingMemReady(hqspi, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != QSPI_OK)
  {
    return QSPI_ERROR;
  }
...
uint8_t W25QXXX_QSPI_DummyCyclesCfg(QSPI_HandleTypeDef *hqspi)
{
  ...
  MODIFY_REG(reg, W25QXXX_VCR_NB_DUMMY, (W25QXXX_DUMMY_CYCLES_READ_QUAD << POSITION_VAL(W25QXXX_VCR_NB_DUMMY)));
...
uint8_t W25QXXX_QSPI_WriteEnable(QSPI_HandleTypeDef *hqspi)
{
  ...
  s_config.Match           = W25QXXX_SR_WREN;
  s_config.Mask            = W25QXXX_SR_WREN;
...
uint8_t W25QXXX_QSPI_WriteDefaultStatusREGs(QSPI_HandleTypeDef *QSPIHandle){
	...
	res = W25QXXX_QSPI_WriteStatusREG(QSPIHandle, 1, 0x00);
	W25QXXX_QSPI_WaitForWriteEnable(QSPIHandle);
	//res = W25QXXX_QSPI_WriteStatusREG(QSPIHandle, 2, 0x00);//By Factory Default QE Bit = 0
	res = W25QXXX_QSPI_WriteStatusREG(QSPIHandle, 2, 0x02); // Enable Memory QSPI QE Bit = 1
	W25QXXX_QSPI_WaitForWriteEnable(QSPIHandle);
	//res = W25QXXX_QSPI_WriteStatusREG(QSPIHandle, 3, 0x60); //By Factory Default Read Driver Strength = 25% (0x60)
	res = W25QXXX_QSPI_WriteStatusREG(QSPIHandle, 3, 0x00); //Set Read Driver Strength = 100%
	W25QXXX_QSPI_WaitForWriteEnable(QSPIHandle);
...
static void W25QXXX_QSPI_WaitForWriteEnable(QSPI_HandleTypeDef *hqspi){
	...
	while(!(reg & 0x02)){
		W25QXXX_QSPI_WriteEnable(hqspi);
		W25QXXX_QSPI_WaitForBusy(hqspi);
		reg = W25QXXX_QSPI_ReadStatusREG(hqspi, 1);
	}
...
static void W25QXXX_QSPI_WaitForBusy(QSPI_HandleTypeDef *hqspi){
	...
	uint8_t reg = W25QXXX_QSPI_ReadStatusREG(hqspi, 1);
	while(reg & 0x01){
		reg = W25QXXX_QSPI_ReadStatusREG(hqspi, 1);
	}
}
