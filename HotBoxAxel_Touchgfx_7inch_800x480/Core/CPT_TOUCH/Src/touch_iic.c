...
****************************************************************************************/
void Touch_IIC_SDA_Config(uint8_t dir)
{
	GPIO_InitTypeDef GPIO_InitStruct = {0};
	GPIO_InitStruct.Pin 			= Touch_IIC_SDA_PIN;				// SCL pin
	GPIO_InitStruct.Speed 		= GPIO_SPEED_FREQ_LOW;			// speed class
	if(dir==1)
	{
		GPIO_InitStruct.Mode 		= GPIO_MODE_INPUT;			// open drain output
		GPIO_InitStruct.Pull 		= GPIO_NOPULL;						// without pull-up

	}
	else
	{
		GPIO_InitStruct.Mode 		= GPIO_MODE_OUTPUT_PP;			// open drain output
		GPIO_InitStruct.Pull 		= GPIO_NOPULL;						// without pull-up
...
*********************************************************************************************/
//480MHz:cpu clock
//240MHz clock=>each count=4 operation=60MHZ=> 1us=60 cnt
